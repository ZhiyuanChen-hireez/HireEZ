...
async def get_embedding(text, model="text-embedding-3-large", max_retries=5, initial_delay=1, timeout=20):
    start = time.time()
    ...
    while retries < max_retries:
        task = asyncio.create_task(client.embeddings.create(input=text, model=model))
        try:
...
def process_education(target_embedding, similar_embedding):
...
async def calculate_similarity_weighted(target_education, similar_education):
    ...
    return process_education(target_embedding, similar_embedding)
...
def process_position(target_position_list, similar_position_list):
...
async def calculate_position_all_comb(target, similar):
    ...
    return process_position(target_position_list, similar_position_list)

def get_additional_education_info(e):
    education_info = {
        'education_school': e['education_school'] if e['education_school'] != "" else " ", 
        'education_degree': e['education_degree_level'] if e['education_degree_level'] != "" else degree_normalization(e['education_degree']), 
        ...
        'education_string': string_from_education([e])
    }
    return education_info

def get_additional_position_info(pos):
    result = {
            'position_company_name': pos['position_company_name'] if pos['position_company_name'] else ' ',
            'position_end_date': 0,
            'position_location': pos['position_location'] if pos['position_location'] else ' ',
...
async def get_profile_embeddings(profile):
    ...
    for e in profile['education']:
        ...
        education_info = get_additional_education_info(e)
    ...
    for p in profile['position']:
        ...
        # initialize other information needed in the results
        position_info = get_additional_position_info(p)
...
async def calculate_profile_similarity(profile1, profile2):
    (education1, position1), (education2, position2) = await asyncio.gather(get_profile_embeddings(profile1), get_profile_embeddings(profile2))
    education_mapping = process_education(education1, education2)
    position_mapping = process_position(position1, position2)
    result = {
...
async def process(data, max_concurrent = 50):
    ...
    tasks_target_profile = [sem_task(calculate_profile_similarity(profile[0], profile[1])) for profile in flattened_data_profile]

    # results = await asyncio.gather(*tasks_target_profile, *tasks_similar_profile)
...
async def test():
    ...
    async def sem_task(task):
        async with semaphore:
            return await task
    print(len(data))
    await asyncio.gather(*[(sem_task(get_profile_embeddings(profile['target']))) for profile in data])  

async def test2():
    results = await calculate_profile_similarity(data[0]['target'], list(data[0]['similar'].values())[0])
    print(results)
    return 0

if __name__ == "__main__":
    # start = time.time()
    # results = asyncio.run(process(data, 80))
    # end = time.time()
    # print(f"Total time taken: {end - start}")
    start = time.time()
    result = asyncio.run(process(data[:10]))
    print(result)
    end = time.time()
